...
%There a number of different data types supported by \DataSeries{}.  In
%particular these are:
%
%\begin{enumerate}
%\item bool -- 0 or 1
%\item byte -- 0 ... 255
%\item int32 -- signed 32bit integer
%\item int64 -- signed 64bit integer
%\item double -- IEEE 64 bit floating point
%\item variable32 -- variable length data up to $2^{31}$ bytes.\footnote{This could be changed to $2^{32}$ bytes by use of unsigned instead of signed.} This is typically used for recording string values.
%\end{enumerate}
...
% Currently users have to specify packing options.
%Section~\ref{sec:lessons} discusses the possibility of auto-inferring 
%the best options.
...
%that if {\it field-name} is the same as the current field's name, the
%previous row's value will be subtracted from the current row's value
%before the data is compressed, and otherwise from the value of the
%other field in the same row.  This feature is only supported for
%int32, int64, and double fields.  For double fields it is required
%that the fields be packed with pack\_scale as well to eliminate
%precision issues. 
%This option is useful, for example, in trace data
%which contains time stamps.  If stored as a double, these timestamps
%effectively have a lot of random bits until they are delta encoded.
