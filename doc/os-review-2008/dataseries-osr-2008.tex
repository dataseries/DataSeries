...
changes we have introducted a version number associated with a type.
...
available in the DataSeries technical
report~\cite{DSTechnicalReportSnapshot} and the source distribution.
...
Access efficiency turns out to be more difficult to measure.  Our most
...
less cpu time.  The reason turned out to be a combination of fread
...
code.  Above we saw the difference in performance that results from
having fields that handle nullable and non-nullable data.  We
experimented further with this for the HP-UX block I/O traces.  We
...
general fields were yet another 1\% slower.  The relatively minor
difference between the performance was because each field was used
