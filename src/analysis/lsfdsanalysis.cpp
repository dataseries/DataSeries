...
lsfdsanalysis - analyze the LSF accounting files converted to DataSeries

=head1 SYNOPSIS

 % lsfdsanalysis I<analysis selection options> input.ds...

=head1 DESCRIPTION

lsfdsanalysis analyzes the LSF accounting files generated by bacct2ds.  It used to be able to
perform a variety of analysis, but most of them were not tested, so have been disabled.  The list
of available modules is shown below:

=head1 OPTIONS

=over 4

=item -a <granularity>:<start-secs>:<end-secs>:<rollup-groups> 

Calculate overall load across a cluster.  Load can be rolled up by a variety of groups.  That
parameter is * or group,group,... where group is: all, production, sequence, team, queue,
hostgroup, team_group, cluster, username, exechost.  Specifying the group erstest will cause ers
testing jobs to be included; they would normally be excluded since they were jobs that were only
run to test the cluster in the animation accounting traces.

=item -b <metaid> # trace a meta-id; valid to specify multiple times

Trace through the set of lsf jobs that all have the same meta-id (id that identifies a group of
requests.

=item -c <start-secs>:<end-secs>

Calculate a job report for a range of seconds.

=item -d <rrid> | T:<team> | U:<user-id> | [AW]:<start-secs>:<end-secs>

Lookup a single render request by id, team, user or time window. Will also find associated metaids,
W spec constrains Team/User searches.  Options rrid, T:, U: are unioned together, and then
intersected by W: If only A: is given, all entries in the window are shown.  The default window is
current-time - 1 day .. now.
...
Look up a meta-id and extract the related information

=item -g 

Attempt to estimate the remaining(?) execution time on jobs that are still running.

=item -h 

Output the summar help message

=item -i 

Calculate a report on host availability.

=item -j

Analyze the complexity of various shots.

=item -k

Calculate job statisticts.

=item -l I<show-finished-jobs>

Identify LSF jobs that are running unexpectedly slowly.  If I<show-finished-jobs> is yes, include
jobs that have finished running, otherwise if no, ignore them.

=item -m I<reference-host> 

Attempt to calibrate the relative performance of hosts based on the specified reference host.

=item -n window_start:window_end:I<options>

Options is a comma list of {purchase_start, reserved_cpus, best_effort_cpus}

=back

Options -[b,d] operate on the r[rj].ds files, the rest operate on the lsf files, and so are
disabled because those inputs are not publically available.

=head1 BUGS

Most of the analysis are currently disabled (with a fatal error) because they have not been
re-tested and added to the regression suite.
