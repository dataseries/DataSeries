...
public:
    ...
    UnionModule(const vector<UM_UnionTable> &sources, const vector<string> &order_columns,
                const string &output_table_name) 
        : output_series(), sources(sources), order_columns(order_columns), 
          output_table_name(output_table_name)
    {
    ...
    virtual Extent *getExtent() {
        ...
        while (true) {
            ...
            for(uint32_t i = 0; i < sources.size(); ++i) {
                if (sources[i].series.getExtent() != NULL) {
                    best = i;
                    break;
                }
            }
            ...
            if (best == numeric_limits<uint32_t>::max()) {
                break;
            }
