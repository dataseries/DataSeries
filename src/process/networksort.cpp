    ...
    void initialize(uint32_t partition_count) {
        ...
        for (uint32_t i = 0; i < partition_count - 1; ++i) {
            ...
            while (partitions.size() < top) {
                LintelLogDebug("networksort", boost::format("partitions[%s] == %s") % partitions.size() % i);
        ...
        while (partitions.size() < 256) {
            LintelLogDebug("networksort", boost::format("partitions[%s] == %s") % partitions.size() % (partition_count - 1));
...
int main(int argc, char *argv[]) {
    ...
    LintelLogDebug("networksort", boost::format("Node index: %s") % node_index);
    ...
    NetworkClique<FixedWidthField, FixedWidthFieldPartitioner>
            network_clique(&input_module, node_names, node_index, field_name_option.get(), FixedWidthFieldPartitioner());
    network_clique.start();

    //ParallelSortModule
    //        sort_module(network_input_module, field_name_option.get(), 10 << 20, -1, 1500 * (1 << 20), false, "/opt/array/tmp/sort");
